...
- Clone this repo
- Edit `chirpstack-gateway-bridge.toml` and add the MQTT password and check topics
- Edit the correct region toml and set the event topics
- Set the MQTT password in `chirpstack` section of `docker-compose.yml`

# MQTT password setup

- Run `docker exec -it chirpstack-docker-mosquitto-1 sh`
- Run `mosquitto_passwd /mosquitto/config/passwords <username>` to add a user
- Make sure to add a `chirpstack` user
